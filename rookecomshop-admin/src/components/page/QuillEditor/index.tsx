...
const QuillEditor = ({
			...
			if (source == 'api') {
				if (quillInstance.current) {
					if (onChange) {
						...
						const textContent = quillInstance.current.root.textContent ?? '';
	...
	useEffect(() => {
		...
		return () => {
			if (quillInstance.current) quillInstance.current.off('text-change', () => {});
		};
	}, [placeholder, readOnly, value, handleTextChange]);
	return (
